defmodule ControlFinancerServerWeb.CreditCardController do
  ...
  def index(conn, _params) do
    credit_cards = Banks.list_credit_cards()
    render(conn, "index.json", credit_cards: credit_cards)
  end

  def create(conn, %{"credit_card" => credit_card_params}) do
    with {:ok, %CreditCard{} = credit_card} <- Banks.create_credit_card(credit_card_params) do
      conn
      |> put_status(:created)
  ...
  def delete(conn, %{"id" => id}) do
    ...
    with {:ok, %CreditCard{}} <- Banks.delete_credit_card(credit_card) do
